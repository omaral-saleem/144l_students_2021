...
A1: All treatments sated around the same amount of abundance. As time goes on each treatment reacts differently. The control treatment had a linear growth. the kelp exudate had a more increase than the control treatment but it has a little reduce in day 2 then it increased and both that treatment and the control ended up around the same amount of cell abundance. The kelp Exudate and nutrients treatment had the most increase in the cell abundance by the end but it also had a little reduce around day 2. Last;ey, the glucose treatment started with very rapid increase in the cell abundance compared to others but it was the only treatment that showed a noticeable decreases in the abundance of the cells after day 2.
...
A2: the biovolume only had an increase for the kelp and nutrients treatment. biovolume showed that all experiment had a rapid growth until day 2 then decreased to almost where it started at day 0 except the kelp and nutrients treatment. The cell abundance data only showed a decrease in the glucose treatment where the cell biovoulme data shows a decrease in all treatments at different levels.
...
A3: the limitations is that we cant account the lag phase because we only have three time points.
...
A4: the values of the kelp exudate treatment decreased while the values of the kelp exudate and nutrients treatment increased. However, the values fot the glucose and control treatments remained the same when we account for changes in cell biovolume.
...
A5: yes, all the patterns line up with the previous observations of the cell abundance including the oxygen grawndown data.
...
A6: a) 10 umol cL^-1
    b) no, only in the Glucose treatment
    c) we cant say how the different types of DOC will affect the TOC concentration
    d) TOC decreases overtime which aligns with the other data because the carbon is being used overtime and respired as co2.
...
A7: the biovolume change doesnt seem to have interpretation of the ∆DOC nor the bioavaliable DOC. However, there is an increases in the BGE when you account for the biovolume which means that not taking the biovolume data can underestimates the BGE.